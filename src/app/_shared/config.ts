export const SEND_EMAIL_INTERNAL = 'https://tybo.co.za/api/api/email/email.php';
export const SEND_EMAIL_INVOICE = 'https://tybo.co.za/api/api/email/email-customer-invoice.php';
export const SEND_EMAIL_RESET_PASSWORD = 'https://tybo.co.za/api/api/email/email-reset-password.php';
export const COMMON_CONN_ERR_MSG = 'it looks like there is an internet connection problem.';
export const BULK_SMS_API = 'https://api.bulksms.com/v1/messages';
export const BULK_SMS_USER = 'ndusystems';
export const BULK_SMS_KEY = 'Harder01!';
export const STRG_ROLE = 'role';
export const STRG_USER = 'user';
export const INVOICE_URL = '0c4c3f54-22f8-11ea-bb26-48f17f8d4d88/48f1/48f17f8d4d88.php';
export const QUOTATION_URL = '0c4c3f54-22f8-11ea-bb26-48f17f8d4d88/48f1/55878781.php';
export const CREDIT_NOTE_URL = '0c4c3f54-22f8-11ea-bb26-48f17f8d4d88/48f1/558787833.php';
export const PURCHASE_ORDER_URL = '0c4c3f54-22f8-11ea-bb26-48f17f8d4d88/48f1/48f17.php';
export const COMBINED_TOKEN = 'A35503D9B94F46AE8C3F44974A120B6D-01-7:t*_SPd4mY00KGYlgOp8HdlDL4Inny';
export const ORDER_SELL = 'Sell';
export const ORDER_PURCHASE = 'Purchase';
