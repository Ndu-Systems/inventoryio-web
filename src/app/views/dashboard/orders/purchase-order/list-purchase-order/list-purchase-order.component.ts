...
import { Orders, NotFoundModel } from 'src/app/_models';
...
import { ORDER_PURCHASE } from 'src/app/_shared';
...
export class ListPurchaseOrderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ordersService.getOrdersForType(user.CompanyId).subscribe(data => {
      if (data) {
        this.orders = data.filter(x => x.OrderType === ORDER_PURCHASE);
        this.ordersService.updatOrdersState(this.orders);
        this.ordersService.loadCurrentOrder(data, ORDER_PURCHASE);
      }
    });
